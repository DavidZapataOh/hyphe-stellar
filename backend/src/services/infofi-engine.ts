...
function analyzeMarketSignals(market: {
  ...
  question: string;
  priceHistory: Array<{ yesPrice: number; timestamp: Date }>;
}): Signal[] {
...
async function computeSignals(): Promise<void> {
  ...
  if (totalSignals > 0) {
    logger.info(
      { signals: totalSignals, markets: teamMarkets.length },
      "InfoFi engine: signals computed"
    );
  }
}
 ...
 */
export function startInfoFiEngine(): void {
