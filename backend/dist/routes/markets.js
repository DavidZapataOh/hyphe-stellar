...
export async function marketRoutes(app) {
    ...
    app.get("/api/markets", async (request) => {
        const { status, sport, sort } = request.query;
    ...
    app.get("/api/markets/:id/quote", async (request) => {
        const { id } = request.params;
        const { outcome, shares } = request.query;
        const cost = await quoteBuy(parseInt(id, 10), parseInt(outcome || "0", 10), BigInt(shares || "1000000"));
