    ...
    switch (parsed.type) {
        case "trade": {
            ...
            if (prices.length >= 2) {
                ...
                await cachePublish(`odds:${d.marketId}`, JSON.stringify({ yes: yesPrice, no: noPrice, timestamp: Date.now() }));
            }
            logger.info({ marketId: d.marketId, side: d.side, user: d.user }, "Indexed trade");
            break;
        }
        case "market_created": {
            const d = parsed.data;
        ...
        case "resolved": {
            ...
            logger.info({ marketId: d.marketId, outcome: d.winningOutcome }, "Indexed market resolution");
            break;
        }
        case "deposit":
        case "withdraw":
